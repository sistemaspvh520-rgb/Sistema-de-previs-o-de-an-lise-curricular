...
export default async function ManagementPage() {
  ...
  return <>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-5">
      ...
      <Card id="atividade-por-usuario" className="overflow-hidden shadow-sm xl:col-span-3"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Users className="size-4 text-brand-cyan-700" /> Atividade por usuário</CardTitle></CardHeader><CardContent className="p-0"><div className="divide-y">{users.map((user) => { const total = totalByUser.get(user.id) ?? 0; const completed = deliveredByUser.get(user.id) ?? 0; const thisMonth = monthlyByUser.get(user.id) ?? 0; const latest = user.analyses[0]; return <div key={user.id} className="grid gap-3 px-4 py-4 text-sm sm:grid-cols-[minmax(150px,1.2fr)_auto_auto] sm:items-center sm:px-6 lg:grid-cols-[minmax(150px,1.2fr)_auto_auto_auto_minmax(170px,1fr)]"><div className="min-w-0"><div className="truncate font-medium">{user.name}</div><div className="text-xs text-muted-foreground">{user.role === "ADMIN" ? "Administrador" : user.role === "ANALYST" ? "Analista" : "Visualizador"}</div></div><div className="flex flex-wrap gap-x-3 gap-y-1 text-xs sm:contents"><span className="whitespace-nowrap">{total} análises</span><span className="whitespace-nowrap">{thisMonth} no mês</span><span className="whitespace-nowrap text-status-success">{completed} prontas</span></div><div className="min-w-0 text-xs text-muted-foreground">{latest ? <><span className="block truncate">{latest.courseName ?? "Curso não identificado"}</span><span>{formatDateTime(latest.createdAt)}</span></> : <>Sem análise · último acesso {formatDateTime(user.lastLoginAt)}</>}</div></div>; })}</div></CardContent></Card>
    </div>
    <div className="mt-6 grid gap-6 xl:grid-cols-5">
