...
export default async function AnalysesPage({
  ...
  const poloCode =
    typeof params.polo === "string" &&
    settings.polos.some((polo) => polo.code === params.polo)
      ? params.polo
      : undefined;
  ...
  return (
    <>
      <PageHeader
        title="Análises"
        description={
          pluralize(total, "análise registrada", "análises registradas") + "."
        }
        actions={
          can(user.role, "analysis:create") && (
            <Button asChild>
              <Link href="/analyses/new">
                <FilePlus2 className="size-4" /> Nova análise
              </Link>
            </Button>
          )
        }
        ...
        <div className="flex min-w-0 flex-wrap gap-1.5">
          <Link
            ...
            className={cn(
              ...
              followUpDue
                ? dueTotal > 0
                  ? "animate-pulse border-status-danger bg-status-danger font-semibold text-white shadow-sm motion-reduce:animate-none"
                  : "border-status-warning bg-status-warning text-white"
                : dueTotal > 0
                  ? "animate-pulse border-status-danger bg-status-danger-bg font-semibold text-status-danger shadow-sm motion-reduce:animate-none"
                  : "border-status-warning/40 bg-status-warning-bg text-status-warning hover:bg-status-warning/20",
            )}
          >
            Resultado a atualizar{dueTotal > 0 ? ` (${dueTotal})` : ""}
          </Link>
        ...
        <form className="flex w-full flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <div className="relative w-full sm:max-w-sm">
            ...
            <Input
              name="q"
              defaultValue={q}
              placeholder="Buscar por aluno, curso ou arquivo"
              className="pl-9"
              aria-label="Buscar por aluno, curso ou arquivo"
            />
          </div>
          <div className="relative w-full sm:w-64">
            <select
              name="polo"
              defaultValue={poloCode ?? ""}
              aria-label="Filtrar por polo"
              className="h-10 w-full appearance-none rounded-lg border border-input bg-card py-2 pr-10 pl-3 text-sm shadow-xs outline-none transition-colors focus:border-ring focus:ring-3 focus:ring-ring/50"
            >
              <option value="">Todos os polos</option>
              {settings.polos.map((p) => (
          ...
          <Button type="submit" variant="outline" className="h-10 sm:shrink-0">
            Filtrar
          </Button>
        </form>
      </div>
              ...
              <TableRow>
                ...
                >
                  ...
                  {statusGroup
                    ? " neste filtro"
                    : status !== "ALL"
                      ? ` com status "${ANALYSIS_STATUS_LABELS[status]}"`
                      : ""}
                  .
                </TableCell>
              </TableRow>
                  ...
                  <Link href={`/analyses/${a.id}`} className="block min-w-0">
                    ...
                    {a.studentName && (
                      <div className="truncate text-xs text-foreground/80">
                        {a.courseName ?? "Curso não identificado"}
                        {a.courseFormat
                          ? ` · ${formatCourseFormat(a.courseFormat)}`
                          : ""}
                        {a.poloCode ? ` · Polo ${a.poloCode}` : ""}
                      </div>
                    )}
                    <div
                      className="truncate text-xs text-muted-foreground"
                      title={a.document?.originalName ?? undefined}
                    >
                      {a.document?.originalName ?? "Arquivo não disponível"}
                    </div>
                    {a.candidateLabel && (
                      <div className="mt-0.5 truncate text-xs text-muted-foreground">
                        Candidato: {a.candidateLabel}
                      </div>
                    )}
                    {showOwner && (
                      <div className="mt-1 flex items-center gap-1 text-xs text-muted-foreground xl:hidden">
                        <UserRound className="size-3" /> Responsável:{" "}
                        {a.createdBy.name}
                      </div>
                    )}
                  </Link>
                ...
                {showOwner && (
                  <TableCell className="hidden xl:table-cell">
                    <div className="flex min-w-0 items-center gap-2">
                      <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-brand-navy-50 text-xs font-semibold text-brand-navy">
                        {a.createdBy.name
                          .split(" ")
                          .filter(Boolean)
                          .slice(0, 2)
                ...
                <TableCell>
                  ...
                  {a.status === "COMPLETED" && (
                    <div
                      className={cn(
                        "mt-1 text-[11px]",
                        a.enrollmentStatus === "ENROLLED"
                          ? "text-status-success"
                          : a.enrollmentStatus === "NOT_ENROLLED"
