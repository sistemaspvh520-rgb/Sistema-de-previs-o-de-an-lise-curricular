...
import {
  createAnalysisFromUpload,
  DuplicateDocumentError,
  InvalidReanalysisError,
  StudentAlreadyAnalyzedError,
} from "@/features/analyses/create-analysis";
import { PdfValidationError } from "@/services/pdf/validate";
import {
  assertCurricularAnalysisDocument,
  InvalidCurricularDocumentError,
} from "@/services/pdf/document-classifier";
...
export async function POST(req: Request) {
  ...
  if (!limit.allowed)
    return NextResponse.json(
      { error: `Muitos envios. Aguarde ${limit.retryAfterSeconds}s.` },
      { status: 429 },
    );
  ...
  if (contentLength > (settings.maxUploadMb + 1) * 1024 * 1024) {
    return NextResponse.json(
      { error: `O arquivo excede o limite de ${settings.maxUploadMb} MB.` },
      { status: 413 },
    );
  }
  ...
  const entry = entrySchema.safeParse({
    ...
    courseFormat: form.get("courseFormat"),
    reanalysisOf: form.get("reanalysisOf") || undefined,
  });
  if (!entry.success)
    return NextResponse.json(
      {
  ...
  if (
    !settings.polos.some((polo) => polo.code === poloCode) ||
    !settings.courseFormats.includes(courseFormat)
  ) {
    return NextResponse.json(
      {
        error:
          "Polo ou formato de curso indisponível nas configurações atuais.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const { id } = await createAnalysisFromUpload({
      ...
      poloCode,
      courseFormat,
      reanalysisOfId: reanalysisOf ?? null,
    });
    after(async () => {
      try {
        await runAnalysisPipeline(id);
      } catch (err) {
        logger.error("pipeline.unhandled", {
          analysisId: id,
          err: String(err),
        });
      }
      ...
      return NextResponse.json(
        {
          ...
          existingId: err.existingAnalysisId,
        },
        { status: 409 },
      );
    }
    if (err instanceof StudentAlreadyAnalyzedError)
      return NextResponse.json(
        {
          error: err.message,
          code: "STUDENT_ALREADY_ANALYZED",
          existingId: err.existingAnalysisId,
        },
        { status: 409 },
      );
    if (err instanceof InvalidReanalysisError)
      return NextResponse.json(
        { error: err.message, code: "INVALID_REANALYSIS" },
        { status: 400 },
      );
    logger.error("upload.failed", { err: String(err) });
    const reason =
      err instanceof Error &&
      err.message.includes("Nenhuma versão de regras ativa")
    ...
    return NextResponse.json(
      { error: reason, code: "ANALYSIS_CREATION_FAILED" },
      { status: 500 },
    );
  }
}
