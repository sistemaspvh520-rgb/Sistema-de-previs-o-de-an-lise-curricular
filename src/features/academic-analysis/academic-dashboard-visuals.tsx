        ...
        return <li key={`${step.curriculumPeriod}-${step.term}-${index}`} className="relative pl-10">
          ...
          <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-[0_10px_28px_-26px_rgba(15,42,66,0.55)] sm:px-5">
            ...
            {step.previousSubjects.length > 0 && <details className="group mt-4 overflow-hidden rounded-xl border border-slate-200 bg-slate-50/70 text-sm open:bg-white">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-5 py-3.5 text-slate-800 transition-colors hover:bg-slate-100/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-slate-400 sm:px-6">
                <span className="flex min-w-0 items-center gap-3"><span className="inline-flex min-w-9 items-center justify-center rounded-full bg-slate-200 px-2.5 py-1 text-xs font-bold tabular-nums text-slate-700">{step.previousSubjects.length}</span><span className="leading-5"><span className="block font-semibold">Disciplinas de adaptação</span><span className="mt-0.5 block text-xs font-normal text-slate-500">Alocadas neste semestre previsto</span></span></span>
                <span className="shrink-0 text-xs font-semibold text-slate-600 group-open:hidden">Ver disciplinas <span aria-hidden="true">⌄</span></span>
                <span className="hidden shrink-0 text-xs font-semibold text-slate-600 group-open:inline">Ocultar <span aria-hidden="true">⌃</span></span>
              </summary>
