...
const schema = z
  .object({
    ...
    defaultStartTerm: z
      ...
      .trim()
      .refine(
        (value) => value === "" || isValidTerm(value),
        "Use AAAA.1 ou AAAA.2.",
      ),
    ...
    followUpDefaultCadence: z.enum(["ONCE_DAILY", "TWICE_DAILY"]),
    polos: z
      .array(
        z.object({
      ...
      .refine(
        ...
        "Não repita códigos de polo.",
      ),
    courseFormats: z.array(z.enum(["EAD_DIGITAL", "SEMIPRESENCIAL"])).min(1),
  })
  .refine(
    (data) => data.followUpBusinessStartHour < data.followUpBusinessEndHour,
    {
      message: "O horário inicial deve ser anterior ao final.",
      path: ["followUpBusinessEndHour"],
    },
  );
...
export async function saveGeneralSettingsAction(
  ...
  try {
    ...
    await Promise.all([
      setSystemSetting("institutionName", parsed.data.institutionName),
      setSystemSetting("maxUploadMb", parsed.data.maxUploadMb),
      setSystemSetting("maxPdfPages", parsed.data.maxPdfPages),
      setSystemSetting(
        "defaultStartTerm",
        parsed.data.defaultStartTerm || null,
      ),
      setSystemSetting("polos", parsed.data.polos),
      setSystemSetting("courseFormats", parsed.data.courseFormats),
      setSystemSetting(
        "followUpBusinessStartHour",
        parsed.data.followUpBusinessStartHour,
      ),
      setSystemSetting(
        "followUpBusinessEndHour",
        parsed.data.followUpBusinessEndHour,
      ),
      setSystemSetting(
        "followUpRepeatBusinessDays",
        parsed.data.followUpRepeatBusinessDays,
      ),
      setSystemSetting(
        "followUpDefaultCadence",
        parsed.data.followUpDefaultCadence,
      ),
    ]);
    await recordAudit({
      userId: user.id,
