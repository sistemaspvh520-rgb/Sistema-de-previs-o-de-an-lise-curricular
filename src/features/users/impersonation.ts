...
export async function consumeImpersonationToken(token: string): Promise<{ target: { id: string; email: string; name: string; role: "ADMIN" | "ANALYST" | "VIEWER"; isActive: boolean }; admin: { id: string; name: string; role: string; isActive: boolean } } | null> {
  ...
  return { target: { id: row.target.id, email: row.target.email, name: row.target.name, role: row.target.role, isActive: row.target.isActive }, admin: { id: row.admin.id, name: row.admin.name, role: row.admin.role, isActive: row.admin.isActive } };
}
