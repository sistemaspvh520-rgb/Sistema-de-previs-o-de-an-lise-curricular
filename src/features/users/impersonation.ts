...
export async function consumeImpersonationToken(token: string): Promise<{ target: { id: string; email: string; name: string; role: "ADMIN" | "ANALYST" | "VIEWER"; sessionVersion: number; isActive: boolean }; admin: { id: string; name: string; role: string; isActive: boolean } } | null> {
