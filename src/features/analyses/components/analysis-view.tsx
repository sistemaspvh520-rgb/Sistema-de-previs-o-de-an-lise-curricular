      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex w-full flex-wrap items-center gap-2 sm:ml-auto sm:w-auto sm:justify-end">
          ...
          {pdfAvailable && perms.diagnostics && (
            <Button variant={showPdf ? "secondary" : "outline"} className="hidden xl:inline-flex" onClick={() => setShowPdf((s) => !s)}>
      ...
      {vm.subjects.length > 0 && <div className={cn("grid gap-6", showPdf && "xl:grid-cols-2")}>
        <Tabs value={tab} onValueChange={setTab} className="min-w-0">
          <TabsList className="flex h-auto w-full flex-nowrap justify-start gap-1 overflow-x-auto p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:flex-wrap sm:overflow-visible">
            ...
            <TabsTrigger value="grade" className={TAB}>Grade curricular</TabsTrigger>
            <TabsTrigger value="pending" className={TAB}>Pendências</TabsTrigger>
            <TabsTrigger value="projection" className={TAB}>Previsão</TabsTrigger>
            {perms.diagnostics && <TabsTrigger value="pdf" className={TAB}>PDF original</TabsTrigger>}
            {perms.diagnostics && <TabsTrigger value="audit" className={TAB}>
              Conferência
