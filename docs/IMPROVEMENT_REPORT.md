...
| F7 | | F7 | ✅ **Feito** — convite por e-mail com link, "Esqueci minha senha", link de redefinição pelo admin e troca no 1º acesso. Antes: ✅ Parcial — `/settings/account` permite trocar a própria senha (mín. 12 caracteres). Pendente: redefinição por e-mail. (achado original:autoatendimento de senha) | P1 | M | Página `/settings/account`; token de redefinição com expiração |
